...
/**
 * Servlet implementation class GetVieweeAjax
 */
@WebServlet("/GetVieweeAjax")
public class GetVieweeAjax extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public GetVieweeAjax() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			viewers = viewerdao.findViewees(userId);
		...
		for(int i=0;i<viewers.size();i++){
			viewerIds.add(viewers.get(i).getUserID());
		}
